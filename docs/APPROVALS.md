...
Rebeam tool approvals are a security protocol, not chat messages. Generic
...
only behind the explicit unsafe `rebeam acp --allow-all` flag.
...
remain disabled under the runtime capability gate; Rebeam never falls back to
bypass mode.
