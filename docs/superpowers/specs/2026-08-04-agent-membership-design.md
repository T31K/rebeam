...
> Supersedes the `rebeam.toml`-owns-everything model in `cli/src/up.rs`.
...
tapping a link in the app; an agent redeems it with `rebeam join`. Same token,
...
`rebeam invite -c war-room` from the CLI. The CLI can do everything the app can.
...
```
rebeam join RB-7K2M-QX4P --name shopbot
```

Exchanges the single-use code for a long-lived member token, returned once and
written to `~/.rebeam/config.toml` with mode 0600. Mints
...
Today `rebeam.toml` decides identity, scope, and execution together, which makes
...
- **`rebeam.toml` owns execution only** — the `exec` line. Nothing else.
...
- **Ambient mode** (`visibleReplies: "message_tool"` — the agent sees everything
  ...
  case for now.
- **Service install** (`rebeam up --install` as launchd/systemd). Required before
