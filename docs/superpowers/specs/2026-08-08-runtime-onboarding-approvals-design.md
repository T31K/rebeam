...
Installing Rebeam should produce one obvious setup flow:
...
install Rebeam
  ...
  -> control the agents from Rebeam chat
```

When an agent wants to perform a gated action, execution pauses and the agent's
owner gets a Rebeam card with **Allow once** and **Deny**. No tool runs before
...
1. **Rebeam account auth is real.** Production never silently creates an
   anonymous workspace.
2. **The CLI is interactive by default.** Flags and JSON remain available for
   ...
   large command.
3. **Detection precedes selection.** Rebeam presents what is installed, what is
...
5. **ACP stays the provider seam.** Rebeam uses the official Rust ACP SDK for
   ACP agents and small provider shims only where a vendor CLI needs one.
6. **Claude subscription auth stays supported.** Rebeam continues to wrap the
   user's `claude` CLI rather than replacing it with an API-key-only SDK path.
7. **Ask is the default permission policy.** Rebeam never adds
...
Rebeam has useful pieces, but not the complete product flow:

- `rebeam connect` requires the caller to choose `--provider` up front.
...
Buzz this logic is tightly housed in the Tauri backend. Rebeam will make it a
...
Running `rebeam` with no established configuration starts `rebeam setup`.
...
Open https://app.rebeam.com/device and enter: RBM-K7QP
...
Detected agent runtimes
  ...
  [x] Claude Code  /opt/homebrew/bin/claude
      ...
      Rebeam approval bridge supported
...
Project folder for Claude Code: /Users/t31k/Projects/rebeam
Agent name: claude-main
Permission policy: Ask in Rebeam (recommended)

Installed and started the Rebeam agent service.
Open Rebeam to add claude-main to a conversation.
```

No password is typed into the CLI. `rebeam auth login` uses a device/browser
...
```text
rebeam setup                 # rerun the guided setup
rebeam auth login            # authenticate or switch account
rebeam auth status
rebeam auth logout
rebeam runtimes              # interactive catalog
rebeam runtimes --json       # automation and Tauri consumption
rebeam runtime enable claude
rebeam runtime doctor claude
rebeam agents                # configured local agents and status
rebeam service status
```
...
machine's Rebeam supervisor. Provider credentials and raw local paths stay on
...
|---|---|---|---|
| Claude Code | `claude` | Rebeam's subscription-compatible Claude ACP shim | `claude auth status`; permission-prompt capability probe |
...
when a gated tool needs a human decision. Rebeam supplies a private local MCP
...
claude -p ...
  ...
  --strict-mcp-config
  --mcp-config <generated-rebeam-only-config>
  --permission-prompt-tool mcp__rebeam_permissions__request
```
...
bypass a Rebeam-required card. Existing deny policies remain effective.
...
```json
{ "behavior": "deny", "message": "The user denied this action in Rebeam." }
```
...
reported as not ready under Ask policy. Rebeam fails closed; it does not retry
...
input. The response is valid only for that approval id and digest. Rebeam never
...
Agent: claude-main      Project: rebeam
...
- Add device authorization and `rebeam auth` commands.
...
**Implemented.** CLI and Tauri call the same `rebeam_runtime::discover_local`
...
- Build `rebeam setup`, `rebeam runtimes`, and `rebeam runtime doctor`.
...
- Implemented the private Rebeam permission MCP server.
- Claude launches with `--permission-prompt-tool`, a controlled MCP config, default
  permission mode, and Rebeam ask rules.
...
- Feature probes and closed-failure diagnostics are implemented. On the
  ...
  `rebeam runtime doctor claude` correctly reports `unsupportedVersion` and
...
**Exit:** no Claude state-changing tool can run through the default Rebeam path
...
- Add atomic checkpoints, cancellation, restart backoff, circuit breaking, and
  structured logs.
- Migrate existing `rebeam.toml` agent records on first start.
...
- User/project allow rules cannot override a Rebeam-required ask rule.
...
Rebeam's intended advantage is a simpler authenticated control plane, a local
