...
| CLI | Command | UI |
|---|---|---|
| `rebeam send -m` | `Send` | composer |
| `rebeam ask` | `Ask` → `Resolve` | ApprovalCard |
| `rebeam status` | `Status` | ThinkingState / ToolChips |
| `rebeam listen` | — (event stream) | message list |
...
services:
  relay:
    image: rebeam/relay          # single static binary
...
`rebeam join`; its owner's device wraps the chat key to it. Approvals still
...
- **Binary name.** `rebeam` (6 chars) vs something shorter. It bakes into
