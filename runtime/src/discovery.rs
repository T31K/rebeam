...
async fn discover_one(
    ...
    if !capabilities.enforceable_tool_approvals && definition.capability_help_flag.is_none() {
        diagnostics.push(
            Diagnostic::error(
...
mod tests {
  ...
  auth) exit 0 ;;
        ...
        executable_fixture(
            &logged_out,
            "codex",
            r#"if [ "$1" = "--version" ]; then echo 'codex 1.2.3'; exit 0; fi
echo 'not logged in' >&2
exit 1"#,
        );
        executable_fixture(&logged_out, "codex-acp", "exit 0");
        let reports = discover(fixture_options(&logged_out)).await;
        let codex = reports
            .iter()
            .find(|report| report.id.as_str() == "codex")
            .unwrap();
        assert_eq!(codex.availability, Availability::LoginRequired);
